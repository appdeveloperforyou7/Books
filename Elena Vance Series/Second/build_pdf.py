# -*- coding: utf-8 -*-
"""Interior PDF builder for THE QUIET WIFE — shaped/iconed cards that split gracefully."""
import re, html
...
from reportlab.platypus import (BaseDocTemplate, PageTemplate, Frame, Paragraph,
    Spacer, PageBreak, Preformatted, NextPageTemplate, Flowable)
...
TOOL = {
    ...
    "SMS":        {"accent": "#2C6FB0", "tint": "#DFEAFA", "label": "SMS"},
    "FORUM":      {"accent": "#0E7C7B", "tint": "#D2EEEE", "label": "DM  \u00b7  FORUM"},
    "CHAT":       {"accent": "#6A4FA6", "tint": "#E8E2F4", "label": "AI ASSISTANT"},
    "TRANSCRIPT": {"accent": "#54565E", "tint": "#E7E7EC", "label": "TRANSCRIPT"},
    "WHATSAPP":   {"accent": "#1E8E3E", "tint": "#DCF0DC", "label": "WHATSAPP"},
    "FINDMY":     {"accent": "#B23A48", "tint": "#F6DEE2", "label": "FIND MY"},
}
RADIUS = {"NOTE":12, "SEARCH":6, "SMS":8, "FORUM":8, "CHAT":12, "TRANSCRIPT":1.5, "WHATSAPP":12, "FINDMY":12}

NUMW = ["", "One","Two","Three","Four","Five","Six","Seven","Eight","Nine","Ten","Eleven",
...
def _mag(c,x,y,s):
    ...
    r=s*0.3; cx,cy=x+s*0.42,y+s*0.6
...
def _phone(c,x,y,s):
    c.setFillColor(white); c.setStrokeColor(white); c.setLineWidth(0)
    c.roundRect(x+s*0.3,y+s*0.12,s*0.4,s*0.76,s*0.07,stroke=0,fill=1)
def _pin(c,x,y,s):
    c.setFillColor(white); c.setStrokeColor(white); c.setLineWidth(0); cx=x+s*0.5
    c.circle(cx,y+s*0.62,s*0.24,stroke=0,fill=1)
    p=c.beginPath(); p.moveTo(cx-s*0.22,y+s*0.55); p.lineTo(cx,y+s*0.08); p.lineTo(cx+s*0.22,y+s*0.55); p.close()
    c.drawPath(p,stroke=0,fill=1)
ICONS = {"NOTE":_pencil,"SEARCH":_mag,"SMS":_bubble,"FORUM":_bubble,"CHAT":_spark,"TRANSCRIPT":_scales,"WHATSAPP":_phone,"FINDMY":_pin}
ICON_S = 10
...
def normal_page(canvas, doc):
    canvas.saveState(); canvas.setFillColor(GREY); pg = doc.page
    if pg % 2 == 0:
        canvas.setFont("Gara-I", 9); canvas.drawString(LM, PH - 0.46 * inch, "The Devoted")
...
def mixed_path(c, x, y, w, h, r, tr, br):
    p = c.beginPath()
    if tr:
        p.moveTo(x, y + h - r); p.curveTo(x, y + h, x, y + h, x + r, y + h)
        p.lineTo(x + w - r, y + h); p.curveTo(x + w, y + h, x + w, y + h, x + w, y + h - r)
    else:
        p.moveTo(x, y + h); p.lineTo(x + w, y + h)
    if br:
        p.lineTo(x + w, y + r); p.curveTo(x + w, y, x + w, y, x + w - r, y)
        p.lineTo(x + r, y); p.curveTo(x, y, x, y, x, y + r)
    else:
...
class ShapedCard(Flowable):
    def __init__(self, kind, body_flow, header=True, top_round=True, bottom_round=True, cont_label=False):
        ...
        self.kind = kind; self.body_flow = body_flow
    ...
    def wrap(self, aw, ah):
        self.width = aw; inner_w = aw - 2 * self.hpad
        total = 0; self._bw = []
        for f in self.body_flow:
            _, fh = f.wrap(inner_w, ah); self._bw.append((f, fh)); total += fh
        self._body_h = total
        top = self.header_h if self.header else 0
        cextra = 12 if self.cont_label else 0
        self.height = top + self._body_h + self.vpad + cextra
        return (self.width, self.height)

    def _extras(self, c, W, H):
        if self.kind == 'CHAT':
            c.setFillColor(white); dy = H - self.header_h / 2.0
            for xx in (W - 12, W - 19, W - 26):
                c.circle(xx, dy, 1.6, stroke=0, fill=1)
        elif self.kind in ('WHATSAPP', 'FINDMY'):
            c.setFillColor(self.accent); r = self.radius
            p = c.beginPath(); p.moveTo(r * 0.7, H); p.lineTo(r * 0.7, H - 12); p.lineTo(-5, H - 3); p.close()
            c.drawPath(p, stroke=0, fill=1)
    ...
    def draw(self):
        c = self.canv; W = self.width; H = self.height; r = self.radius
        tr = self.top_round; br = self.bottom_round
        top = self.header_h if self.header else 0
        cextra = 12 if self.cont_label else 0
        bh = H - top - cextra
        # body tint (full card)
        ...
        if self.header:
            ...
            c.drawPath(mixed_path(c, 0, H - self.header_h, W, self.header_h, r, tr, False), stroke=0, fill=1)
        if self.cont_label:
            c.setFillColor(self.accent); c.setFont("Mono-B", 7)
            c.drawString(self.hpad, H - 9, "(continued)")
        # outline
        c.setStrokeColor(self.accent); c.setLineWidth(0.75)
        c.drawPath(mixed_path(c, 0, 0, W, H, r, tr, br), stroke=1, fill=0)
        # header content
        if self.header:
            c.saveState()
            ICONS[self.kind](c, self.hpad, (H - self.header_h) + (self.header_h - ICON_S) / 2.0, ICON_S)
            c.restoreState()
            self._hdr = Paragraph(self.label, HDR)
            _, hh = self._hdr.wrap(W - 2 * self.hpad - self.icon_gap, H)
            self._hdr.drawOn(c, self.hpad + self.icon_gap, (H - self.header_h) + (self.header_h - hh) / 2.0)
            self._extras(c, W, H)
        # body content (top-down within body region)
        y = bh - self.vpad
        for f, fh in self._bw:
            y -= fh; f.drawOn(c, self.hpad, y)

    def split(self, aw, ah):
        self.wrap(aw, ah)
        if self.height <= ah:
            return [self]
        inner_w = aw - 2 * self.hpad
        top = self.header_h if self.header else 0
        cextra = 12 if self.cont_label else 0
        ...
        if avail_body < 42:
            return []
        f = self.body_flow[0]
        parts = f.split(inner_w, avail_body)
...
def classify_code(lines):
    ...
    if first.startswith("note"): return "NOTE"
    if first.startswith("search"): return "SEARCH"
    if first.startswith("transcript"): return "TRANSCRIPT"
    if first.startswith("chat"): return "CHAT"
    if first.startswith("dm") or first.startswith("reply") or first.startswith("r/") or "truecrime" in first: return "FORUM"
    return "NOTE"

def tool_card(lines):
    return [Spacer(1, 13), ShapedCard(classify_code(lines), [Paragraph(code_body_html(lines), CODE)]), Spacer(1, 13)]

def message_card(qlines):
    joined = clean(" ".join(qlines)).lower()
    kind = "FINDMY" if "find my" in joined else "WHATSAPP" if "whatsapp" in joined else "SMS" if "sms" in joined else "WHATSAPP"
...
story = [Spacer(1, 2.3 * inch), Paragraph("THE DEVOTED", TMAIN), Spacer(1, 0.18 * inch),
         Paragraph("The Elena Vance Series &middot; Book Two", TSUB), Spacer(1, 2.6 * inch),
         Paragraph("a novel", TSUB)]

new_section = True; para = []; quote = []; first_chapter = True
def flush_para():
    global para, new_section
    if para:
        story.append(Paragraph(inline(" ".join(para)), BODYF if new_section else BODY)); new_section = False
    para = []
def flush_quote():
    global quote, new_section
    if quote:
        story.extend(message_card(quote)); new_section = True
    quote = []

i = 0; n = len(raw)
while i < n and not (raw[i].startswith("# Chapter ") or raw[i].startswith("# Interlude")):
    i += 1
...
while i < n:
    ...
    if line.strip().startswith("```"):
        ...
        i += 1
        story.extend(tool_card(buf)); new_section = True
        continue
    if line.startswith("# Chapter "):
        flush_para(); flush_quote()
        m = re.match(r"# Chapter (\d+)", line); num = int(m.group(1)) if m else 0
        j = i + 1; title = ""
        ...
        if num < len(NUMW): story.append(Paragraph("Chapter " + NUMW[num], CNUM))
        story.append(Paragraph(inline(title), CTITLE)); story.append(Spacer(1, 0.34 * inch))
        story.append(NextPageTemplate("normal")); new_section = True
    ...
    if line.startswith("# Interlude"):
        ...
        story.append(NextPageTemplate("chapter")); story.append(PageBreak())
        story.append(SetChapter(title)); story.append(Spacer(1, 1.25 * inch))
        story.append(Paragraph("Interlude", CNUM))
        story.append(Paragraph(inline(title), CTITLE)); story.append(Spacer(1, 0.34 * inch))
        story.append(NextPageTemplate("normal")); new_section = True
        i = j + 1 if title else i + 1
        continue
    ...
    if line.strip() == "---":
        ...
        if k < n and (raw[k].startswith("# Chapter ") or raw[k].startswith("# Interlude")): i += 1; continue
        story.append(Paragraph("&lowast; &nbsp; &lowast; &nbsp; &lowast;", SCENE)); new_section = True
        i += 1; continue
    if line.startswith(">"):
        flush_para(); quote.append(line[1:].lstrip()); i += 1; continue
    if line.strip() == "":
        flush_para()
...
doc = BaseDocTemplate(OUT, pagesize=(PW, PH), leftMargin=LM, rightMargin=RM, topMargin=TM, bottomMargin=BM,
    title="The Devoted", author="The Elena Vance Series")
doc.addPageTemplates([
    PageTemplate(id="title", frames=[frame], onPage=blank_page),
