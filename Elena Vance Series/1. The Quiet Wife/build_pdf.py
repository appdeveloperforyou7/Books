...
def normal_page(canvas, doc):
    ...
    if pg % 2 == 0:
        canvas.setFont("Gara-I", 9); canvas.drawString(LM, PH - 0.46 * inch, "The Quiet Wife")
...
def message_card(qlines):
    ...
    kind = "FINDMY" if "find my" in joined else "WHATSAPP"
    body_html = "<br/>".join(inline(x) for x in qlines)
    return [Spacer(1, 13), ShapedCard(kind, [Paragraph(body_html, MSGBODY)]), Spacer(1, 13)]

SRC = r"D:\Kapil\Books\Elena\manuscript.md"
OUT = r"D:\Kapil\Books\Elena\The_Quiet_Wife_interior.pdf"

with open(SRC, encoding="utf-8") as fh:
    raw = fh.read().split("\n")

story = [Spacer(1, 2.3 * inch), Paragraph("THE QUIET WIFE", TMAIN), Spacer(1, 0.18 * inch),
         Paragraph("The Elena Vance Series &middot; Book One", TSUB), Spacer(1, 2.6 * inch),
...
i = 0; n = len(raw)
while i < n and not raw[i].startswith("# Chapter "):
    i += 1
...
while i < n:
    ...
    if line.strip() == "---":
        ...
        if k < n and raw[k].startswith("# Chapter "): i += 1; continue
...
doc = BaseDocTemplate(OUT, pagesize=(PW, PH), leftMargin=LM, rightMargin=RM, topMargin=TM, bottomMargin=BM,
    title="The Quiet Wife", author="The Elena Vance Series")
