...
OUTPUT_DIR = os.path.join(BASE, "German")
OUTPUT_PATH = os.path.join(OUTPUT_DIR, "Die_Vierte_Stufe_German_Interior.pdf")
...
def parse_chapter(filepath):
    ...
    is_prologue = "Prolog" in header_line or "prolog" in filepath.lower()
    is_epilogue = "Epilog" in header_line or "epilog" in filepath.lower()
    is_chapter = not is_prologue and not is_epilogue

    chapter_info = {"prologue": is_prologue, "epilogue": is_epilogue}
    if is_chapter:
        m = re.match(r"# .+? — Kapitel (\d+)\s*:?\s*(.+)", header_line)
        if m:
...
def build_chapter_flowables(chapter_info, elements, styles):
    ...
    if is_prologue:
        flowables.append(Paragraph("PROLOG", styles["chapter_num"]))
        flowables.append(Spacer(1, 18))
    elif is_epilogue:
        flowables.append(Paragraph("EPILOG", styles["chapter_num"]))
        flowables.append(Spacer(1, 18))
    else:
        num = chapter_info.get("number", 0)
        pov = chapter_info.get("pov", "")
        flowables.append(Paragraph(f"KAPITEL {num}", styles["chapter_num"]))
...
def build_part_page(part_info, styles):
    ...
    flowables.append(Paragraph(f"TEIL {part_info['number']}", styles["part_num"]))
    flowables.append(Spacer(1, 12))
...
def build_title_page(styles):
    ...
    flowables.append(Paragraph("DIE VIERTE STUFE", styles["title_main"]))
    flowables.append(Paragraph("Ein Roman", styles["title_sub"]))
...
def build_copyright_page(styles):
    ...
    lines = [
        ...
        "des Verlages in irgendeiner Form reproduziert, verbreitet oder übertragen werden.",
        "",
        "Dies ist ein Werk der Fiktion. Namen, Figuren, Orte und Vorfälle sind entweder",
        "Produkt der Einbildungskraft des Autors oder fiktiv verwendet.",
        "Jede Ähnlichkeit mit tatsächlichen Personen, lebend oder tot,",
...
def build_epigraph_page(styles):
    ...
    flowables.append(Paragraph(
        '&laquo;Der Körper hat keine Agenda. Der Körper wurde nie manipuliert.&raquo;',
        styles["epigraph"]
    ))
    flowables.append(PageBreak())
    return flowables
...
def build_dedication_page(styles):
    flowables = []
    flowables.append(Spacer(1, 3.5 * inch))
    flowables.append(Paragraph("Für diejenigen, die sich erinnern.", styles["epigraph"]))
    flowables.append(PageBreak())
    flowables.append(PageBreak())
    return flowables
...
def main():
    ...
    print("=" * 60)
    print("DIE VIERTE STUFE — Innen-PDF-Generator")
    print("=" * 60)
