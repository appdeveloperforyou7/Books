...
OUTPUT_DIR = SCRIPT_DIR / "Book1" / "Images"
...
CHARACTER_DESCRIPTIONS = {
    ...
    "blip": (
        "a small cute white cube robot named Blip approximately 4 inches per side, matte "
        "white casing with rounded edges and slight scuff marks, front LED screen "
        "displaying simple emoji-style face with two round dot eyes and a curved smile "
        "line in cyan teal color, small antenna on top that wiggles, tiny hover thrusters "
        "underneath floating about 2 feet off ground, slight dirt smudges, friendly "
        "childlike curious appearance"
    ),
    "daadi": (
...
CHAR_REF_DIR = OUTPUT_DIR.parent / "Images" / "Characters"
...
def build_full_prompt(raw_prompt: str) -> str:
    ...
    char_parts = []
    for c in characters:
        if len(characters) > 2 and c in CHARACTER_SHORT:
            char_parts.append(CHARACTER_SHORT[c])
        elif c in CHARACTER_DESCRIPTIONS:
    ...
    char_names = ", and ".join(c.capitalize() for c in characters) if characters else ""
...
def main():
    ...
    for i, item in enumerate(todo, 1):
        ...
        steps = args.steps or STEPS_BY_TYPE.get(item_type, 4)
        chars = detect_characters(raw_prompt)
        seed = compute_seed(chars) if chars else 0
        full_prompt = raw_prompt if args.raw_prompts else build_full_prompt(raw_prompt)
        out_path = get_output_path(item)
